...
##
function _build_tensor_lag_decay(tdata,subset,names;window=7,skip=6,decay=0.5)
  ...
  for tw=1:nT-window+1-skip
    ...
    W = T[:,:,tw+skip:(tw+skip+window-1),:]
    factor = one(typeof(decay))
    for s=reverse(1:size(W,3))
      W[:,:,s,:] *= factor
      factor *= decay
    end
...
T = _build_tensor_lag_decay(tensor1,C1,data["names"])
##
